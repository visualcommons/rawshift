...
pub fn rotate_90_cw(image: &mut RgbImage) {
    ...
    for old_row in 0..old_h {
        for old_col in 0..old_w {
            ...
            new_data[dst] = data[src];
            new_data[dst + 1] = data[src + 1];
            new_data[dst + 2] = data[src + 2];
        }
    }
    image
        .replace_data(new_w as u32, new_h as u32, new_data)
...
pub fn apply_crop(image: &mut RgbImage, crop: Rect) {
    ...
    if x + w <= image.width() as usize && y + h <= image.height() as usize {
        ...
        for row in 0..h {
            ...
            new_data.extend_from_slice(&data[src_base..src_base + w * 3]);
        }
        image
            .replace_data(w as u32, h as u32, new_data)
            .expect("crop copies w*h*3 samples from a bounds-checked region");
    } else {
        tracing::warn!(
...
mod tests {
    use super::*;
    use crate::core::image::{Dimensions, Point};

    fn make_image(w: u32, h: u32, data: Vec<u16>) -> RgbImage {
        RgbImage::new(w, h, data).expect("valid RGB buffer")
    }

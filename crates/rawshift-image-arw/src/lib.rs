...
/// Sony ARW format marker.
pub struct Arw;

#[cfg(feature = "decode")]
impl rawshift_image_core::FormatSniffer for Arw {
    const FORMAT: rawshift_image_core::FormatId = rawshift_image_core::FormatId::Arw;
    fn matches(data: &[u8]) -> bool {
        tiff_make(data).is_some_and(|make| make.to_ascii_lowercase().contains("sony"))
    }
}

#[cfg(feature = "decode")]
impl rawshift_image_core::ImageDecoder for Arw {
    type Options = ();
    type Output = rawshift_image_core::RawImage;
    fn decode(data: &[u8], _: &()) -> rawshift_image_core::RawResult<Self::Output> {
        ArwFile::parse(std::io::Cursor::new(data))?.decode_raw()
    }
}

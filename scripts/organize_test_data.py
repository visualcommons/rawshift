...
def run_dcraw_identify(src_file, dest_dir):
    """Generate dcraw identification info."""
    dest_file = dest_dir / "dcraw_identify.txt"
    if shutil.which("dcraw"):
        cmd = ["dcraw", "-i", "-v", str(src_file)]
        with open(dest_file, "w") as f:
            subprocess.run(cmd, stdout=f, stderr=subprocess.STDOUT, check=False)
...
def organize_and_generate():
    ...
    for raw_file in raw_files:
        ...
        # 3. Generate/Update dcraw identify
        run_dcraw_identify(target_file, fixture_dir)
        ...
        if old_fixture_json.exists():
            print(
                f"  Moving existing manual fixture {old_fixture_json.name} to {target_expected_json.name}"
            )
